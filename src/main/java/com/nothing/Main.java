	...
	public static void main(String[] args) throws InterruptedException {
		...
		while (true) {
			...
			for (int i = 0; i < languages.length; i++) {
				File file =
						new File("urls" + separator + fileName);
				try {
					List<String> strings = IOUtils.readLines(new FileInputStream(file), StandardCharsets.UTF_8);
					int finalI = i;
					strings.forEach(s -> completionService
							.submit(new LanguageParserWorker(String.format(s, languages[finalI].name().toLowerCase()), languages[finalI])));
					queue.remove(take);
				}
				catch (IOException e) {
					e.printStackTrace();
